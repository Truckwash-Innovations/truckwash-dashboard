 ...
 *   npm run selftest
 *
 * Hoe dit bestand is ingedeeld
 * ----------------------------
 *
 * Hier stonden honderd hoofdstukken onder elkaar, dertienduizend regels.
 * De eerste achtentwintig zijn één doorlopend verhaal: inloggen, offline
 * schrijven, de wachtrij, weer online, een dossier, een postbus. Elk
 * hoofdstuk bouwt voort op de toestand van het vorige, en dat is geen
 * slordigheid maar precies wat er getest wordt -- een synchronisatie meet je
 * niet in losse stukjes. Die blijven dus staan, op volgorde.
 *
 * Alles daarna is per reparatie geschreven en staat op zichzelf. Dat deel is
 * verhuisd naar scripts/selftest/, per onderwerp:
 *
 *   kern.ts       de nagebootste browser, check() en de telling
 *   schermen.ts   menu, zoeken, opmaak en de wegen ertussen
 *   mensen.ts     dossiers, rechten, vestigingen, pasjes
 *   post.ts       werkadressen, postvakken, documenten, handtekeningen
 *   factuur.ts    de factuur van binnenkomst tot goedkeuring
 *   exact.ts      sleutels, rekeningschema, boeken en betalen
 *   lezen.ts      wat de AI van een factuur maakt, en wat er misgaat
 *   inkoop.ts     waar facturen binnenkomen
 *   beheer.ts     de site, de wekkers, de versies, en wat los staat
 *
 * De nummers blijven wat ze waren. Ze zijn geen volgorde maar een naam: in
 * commentaar door de hele repo staat "zie groep 89", en dat hoort te blijven
 * kloppen. Doordat ze nu per onderwerp bij elkaar staan, lopen ze in de
...
/* ==================================================================== *
 *  En de rest, per onderwerp
 *
 *  Elk van deze bestanden bevat hoofdstukken die op zichzelf staan: ze
 *  maken hun eigen gegevens en ruimen ze op. De volgorde hier is de
 *  volgorde waarin ze worden afgedrukt.
 * ==================================================================== */

for (const [titel, laad] of [
  ['De schermen',   () => import('./selftest/schermen.ts')],
  ['Mensen',        () => import('./selftest/mensen.ts')],
  ['De post',       () => import('./selftest/post.ts')],
  ['De factuur',    () => import('./selftest/factuur.ts')],
  ['Exact',         () => import('./selftest/exact.ts')],
  ['Het lezen',     () => import('./selftest/lezen.ts')],
  ['De inkoop',     () => import('./selftest/inkoop.ts')],
  ['Beheer',        () => import('./selftest/beheer.ts')],
] as [string, () => Promise<{ groepen: () => Promise<void> }>][]) {
  console.log(`\n========== ${titel} ==========`)
  await (await laad()).groepen()
}

const { passed, failed } = telling()
