-- ===========================================================================
--  Bijwerken: migratie 0017 tot en met 0112
--
...
--    0071  Documentbeheer
--    0072  De administratie komt binnen
--    0073  Een nagekomen wasbeurt blokkeert niet de hele ronde
--    0074  Je eigen woonadres invullen
--    0075  Klanten beheren
--    0076  Wachtwoord vergeten -- een code in plaats van een link naar localhost
--    0077  Een vacature op een vestiging die niet op de website staat
--    0078  De app zei ja en de database nee
--    0079  De bon weet zelf in welke bv hij hoort
--    0080  De AI leest ook een pasje
--    0081  Een eigen werkadres
--    0082  Een postvak per medewerker
--    0083  Documenten maak je hier
--    0084  Gedeelde postvakken, en eigen mappen om in te sorteren
--    0085  De omschrijving in het dagboek
--    0086  Alles per bv
--    0087  De geschiedenis van Exact
--    0088  De koppeling die niemand terugvond
--    0089  Wat Exact zelf al wist
--    0090  Het nummer waarop je een boeking in Exact terugvindt
--    0091  Een boeking die niet optelt gaat niet
--    0092  Niet meer per bv instellen wat Exact al weet
--    0093  Indelen zonder eerst over te nemen
--    0094  Een rekeningnummer dat verkeerd gelezen is
--    0095  Een inkoopadres per onderneming, met een naam eraan
--    0096  De tweede handtekening ligt bij iemand
--    0097  De adressen maken zichzelf
--    0098  Een vestiging hoort bij de bv die zo heet
--    0099  Een weigering die je terugvindt, en die weggaat als je hem oplost
...
--    0104  Eén rekeningschema, en niet drie
--    0105  Wat van de server is, staat in een lijst -- niet in een trigger erbij
--    0106  Een factuur weet bij wie hij hoort
--    0107  Een wekker die je zelf kunt laten afgaan
--    0108  Een werkadres dat op het verkeerde domein staat
--    0109  "Aangenomen" is niet hetzelfde als "gedaan"
--    0110  Meer dan één paar ogen per stap
...
do $stand$ begin
  if to_regprocedure('public.migratie_gedaan(integer,text)') is not null then
    perform public.migratie_gedaan(104, 'Eén rekeningschema, en niet drie');
  end if;
end $stand$;
...
do $stand$ begin
  if to_regprocedure('public.migratie_gedaan(integer,text)') is not null then
    perform public.migratie_gedaan(108, 'Een werkadres dat op het verkeerde domein staat');
  end if;
end $stand$;
...
do $stand$ begin
  if to_regprocedure('public.migratie_gedaan(integer,text)') is not null then
    perform public.migratie_gedaan(109, '"Aangenomen" is niet hetzelfde als "gedaan"');
  end if;
end $stand$;
...
do $stand$ begin
  if to_regprocedure('public.migratie_gedaan(integer,text)') is not null then
    perform public.migratie_gedaan(110, 'Meer dan één paar ogen per stap');
  end if;
end $stand$;
...
do $stand$ begin
  if to_regprocedure('public.migratie_gedaan(integer,text)') is not null then
    perform public.migratie_gedaan(111, 'Een wekker die je ook buiten zijn uur kunt laten afgaan');
  end if;
end $stand$;
...
do $stand$ begin
  if to_regprocedure('public.migratie_gedaan(integer,text)') is not null then
    perform public.migratie_gedaan(112, 'Een mail die sneuvelt is niet verloren');
  end if;
end $stand$;
