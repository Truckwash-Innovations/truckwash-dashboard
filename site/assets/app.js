...
/* De landkaart staat in assets/nlkaart.js, en dat bestand wordt alleen op
   /locaties/ geladen. Hier stond hij als constante NLPROV in app.js -- 21 kB
   die de andere 46 pagina's voor niets meedroegen.

   Die oude kaart kwam van cartomap en had drie problemen die je pas ziet als
   je er echt naar kijkt: het IJsselmeer was land (die bron geeft provincies
   inclusief water, dus liep Noord-Holland door tot Friesland), de
   Waddeneilanden en de Zeeuwse delta waren in de vereenvoudiging verdwenen, en
   het formaat kende geen gaten -- elke ring werd een massief blok, dus een
   meer werd een berg. Zie bouw/kaart.cjs. */
...
function bouwKaart3D(canvas, locs, opKlik) {
  if (!window.THREE || canvas.dataset.klaar) return;
  /* assets/nlkaart.js staat alleen op deze pagina. Ontbreekt hij, dan is een
     lege kaarthouder beter dan een halve kaart of een pagina die stukloopt. */
  const KAART = window.NLKAART;
  if (!KAART || !KAART.length) return;
  canvas.dataset.klaar = "1";
  const kaal = matchMedia("(prefers-reduced-motion: reduce)").matches;
  ...
  const DIEP = 0.85;
  const provincies = [];
  const randMat = new THREE.LineBasicMaterial({
    color: 0x9fbcff, transparent: true, opacity: 0.5 });

  KAART.forEach((prov, pi) => {
    const groep = new THREE.Group();
    const mat = new THREE.MeshStandardMaterial({
      color: TINTEN[pi % TINTEN.length], roughness: 0.66, metalness: 0.28,
    });

    /* Elk stuk is [buitenring, gat, gat, ...]. Een provincie kan uit meerdere
       stukken bestaan -- Fryslân is het vasteland plus vier Waddeneilanden --
       en er zitten meren in als gat. Zonder die gaten wordt een meer een
       massief blok dat boven het land uitsteekt. */
    prov.p.forEach(stuk => {
      const vorm = new THREE.Shape();
      stuk[0].forEach(([x, z], i) => (i ? vorm.lineTo(x, -z) : vorm.moveTo(x, -z)));
      vorm.closePath();
      for (let k = 1; k < stuk.length; k++) {
        const gat = new THREE.Path();
        stuk[k].forEach(([x, z], i) => (i ? gat.lineTo(x, -z) : gat.moveTo(x, -z)));
        gat.closePath();
        vorm.holes.push(gat);
      }

      const geo = new THREE.ExtrudeGeometry(vorm, {
        depth: DIEP, bevelEnabled: true, bevelSize: 0.05, bevelThickness: 0.05, bevelSegments: 1,
      });
      ...
      /* De rand als losse lijn over de bovenkant, en niet met EdgesGeometry.
         Die laatste zoekt scherpe hoeken in de hele extrusie en levert bij een
         kust van vijfduizend punten tienduizenden lijnstukjes op -- waarvan de
         helft op de zijkanten zit waar je ze niet ziet. Dit is precies de
         kustlijn, één segment per punt. */
      stuk.forEach(ring => {
        const pos = new Float32Array(ring.length * 3);
        /* Let op het teken. De extrusie wordt -90 graden om de x-as gedraaid,
           en daardoor komt een vormpunt (x, -z) in de wereld uit op (x, ., z).
           Met -z hier tekende de kustlijn een tweede Nederland ondersteboven
           over de eerste heen -- en dat zag eruit als provincies die niet
           waren opgekomen. */
        ring.forEach(([x, z], i) => {
          pos[i * 3] = x; pos[i * 3 + 1] = DIEP + 0.012; pos[i * 3 + 2] = z;
        });
        const lg = new THREE.BufferGeometry();
        lg.setAttribute("position", new THREE.BufferAttribute(pos, 3));
        groep.add(new THREE.LineLoop(lg, randMat));
      });
    });
  ...
  /* ---- de plaatsnamen ----
     ...
     Een kaart met achttien gele spelden zegt "we zitten door het hele land";
     een kaart met de namen erbij zegt "we zitten bij jou om de hoek". Dat
     tweede is waar iemand op deze pagina naar zoekt.

     Als HTML boven het canvas en niet als plaatje in de 3D-wereld: tekst in
     een texture wordt wazig zodra je inzoomt, en deze naampjes moeten leesbaar
     zijn -- dat is het hele punt. Ze worden elk beeldje meegerekend met
     dezelfde projectie die het zwevende label ook gebruikt.

     Achttien namen passen niet allemaal naast elkaar in een vlak van 500
     pixels: Amstelveen en Amsterdam liggen tien pixels uit elkaar. Daarom
     wijkt wat botst. Wie het dichtst bij de camera staat en wat je aanwijst
     hebben voorrang -- zo verdwijnt nooit de naam waar je naar kijkt. */
  const naamLaag = document.createElement("div");
  naamLaag.className = "kaartnamen";
  naamLaag.setAttribute("aria-hidden", "true");
  houder.appendChild(naamLaag);

  const naampjes = spelden.map(g => {
    const el = document.createElement("span");
    el.className = "kaartnaam";
    el.textContent = g.userData.loc.plaats;
    naamLaag.appendChild(el);
    return { el, g, breed: 0, hoog: 0, zicht: false };
  });
  /* Één keer meten, buiten de tekenlus: offsetWidth opvragen dwingt de browser
     tot een herberekening van de opmaak, en achttien keer per beeldje is dat
     het verschil tussen soepel en schokkerig. */
  naampjes.forEach(n => { n.breed = n.el.offsetWidth; n.hoog = n.el.offsetHeight; });
  ...
  /* Afgeleid van de werkelijke omvang van het land en niet als vast getal.
     Hier stond 46, ingesteld op een kaart die kleiner was; met de echte
     kustlijn erbij -- Wadden erboven, Zeeland eronder -- klopte dat niet meer
     en bleef een derde van het vlak leeg. Nu schuift de camera mee als de
     kaart verandert.

     De waarden worden hieronder gezet, zodra R bekend is. */
  let PAS, MIN_Z, MAX_Z;
  ...
  PAS = R * 3.25; MIN_Z = R * 1.9; MAX_Z = R * 6.6;
  zoomDoel = PAS;
  ...
  function tik() {
    ...
    /* plaatsnamen op hun plek zetten, en laten wijken wat botst */
    if (naampjes.length) {
      const r = canvas.getBoundingClientRect();
      const kandidaten = [];
      naampjes.forEach(n => {
        scherm.copy(n.g.position); scherm.y += 2.05; scherm.project(cam);
        /* achter de camera, of buiten beeld: overslaan */
        if (scherm.z > 1) { n.zicht = false; return; }
        const x = (scherm.x * 0.5 + 0.5) * r.width;
        const y = (-scherm.y * 0.5 + 0.5) * r.height;
        if (x < -40 || y < -20 || x > r.width + 40 || y > r.height + 20) { n.zicht = false; return; }
        const aan = n.g.userData.i === hover || n.g.userData.i === gekozen;
        kandidaten.push({ n, x, y, aan, diepte: scherm.z });
      });

      /* Voorrang: wat je aanwijst eerst, daarna wat vooraan ligt. */
      kandidaten.sort((a, b) => (b.aan - a.aan) || (a.diepte - b.diepte));

      /* Vijf plekken rond de speld, op volgorde van voorkeur. Alleen "boven"
         aanbieden en de rest laten vallen kostte zeven van de achttien namen:
         in de Randstad liggen de vestigingen dicht op elkaar en dan botst
         alles met alles. Met een uitwijkplek links of rechts passen ze bijna
         allemaal, en pas als geen van de vijf vrij is verdwijnt er een. */
      const PLEKKEN = [
        (x, y, w, h) => [x - w / 2, y - h - 4],       // boven
        (x, y, w, h) => [x + 11, y - h / 2],          // rechts
        (x, y, w, h) => [x - w - 11, y - h / 2],      // links
        (x, y, w, h) => [x + 9, y - h - 10],          // rechtsboven
        (x, y, w, h) => [x - w - 9, y - h - 10],      // linksboven
      ];

      const bezet = [];
      const vrij = (l, t2, w, h) => !bezet.some(v =>
        !(l + w + 4 < v[0] || l - 4 > v[2] || t2 + h + 4 < v[1] || t2 - 4 > v[3]));

      kandidaten.forEach(k => {
        const w = k.n.breed, h = k.n.hoog;
        let gekozenPlek = null;
        for (const plek of PLEKKEN) {
          const [l, t2] = plek(k.x, k.y, w, h);
          if (l < 2 || t2 < 2 || l + w > r.width - 2 || t2 + h > r.height - 2) continue;
          if (vrij(l, t2, w, h)) { gekozenPlek = [l, t2]; break; }
        }
        /* Wat je aanwijst wijkt nooit: dan maar over een buurman heen. */
        if (!gekozenPlek && k.aan) gekozenPlek = PLEKKEN[0](k.x, k.y, w, h);
        if (!gekozenPlek) { k.n.zicht = false; return; }

        const [l, t2] = gekozenPlek;
        bezet.push([l, t2, l + w, t2 + h]);
        k.n.el.style.transform = "translate(" + Math.round(l) + "px," + Math.round(t2) + "px)";
        k.n.el.classList.toggle("aan", k.aan);
        k.n.zicht = true;
      });

      naampjes.forEach(n => n.el.classList.toggle("weg", !n.zicht));
    }

    /* Het zwevende label boven een speld die je aanwijst blijft: het staat
       hoger dan de naampjes en is de bevestiging dat je die speld te pakken
       hebt. */
    if (label) {
      if (hover >= 0) {
