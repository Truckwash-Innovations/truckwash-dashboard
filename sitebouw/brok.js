...
const BEELD = JSON.parse(document.getElementById("beelddata").textContent);
const IMG = (rol, alt, cls) => {
  const b = BEELD[rol];
  if (!b) return "";
  return `<img src="${b.uri}" width="${b.w}" height="${b.h}" alt="${esc(alt)}"${cls ? ` class="${cls}"` : ""} loading="lazy">`;
};
...
/* Drie takken, en de tests zijn streng (=== true, === "werkplek") en niet op
   waarheid. De webshopregel hierboven houdt zijn letterlijke true en dus exact
   dezelfde opmaak; met een test op waarheid zou "werkplek" in diezelfde tak
   vallen en een gele knop met pijltje in een nieuw tabblad opleveren. */
document.getElementById("menu").innerHTML = MENU.map(([h, t, soort]) =>
  soort === true         ? `<a class="webshop" href="${h}" target="_blank" rel="noopener">${t} <span aria-hidden="true">&#8599;</span></a>`
  : soort === "werkplek" ? `<a class="werkplek" href="${h}">${t}</a>`
  : `<a href="${h}">${t}</a>`).join("");
...
/* De onderbalk voor de telefoon. In de preview met de algemene adressen; op de
   site vult webbouw.cjs hem per pagina in, met op een vestigingspagina de route
   en het nummer van die vestiging. Zie onderbalk() hieronder. */
const onderbalkEl = document.getElementById("onderbalk");
if (onderbalkEl) onderbalkEl.innerHTML = onderbalk({
  locatie: "#/locaties", route: "#/locaties", bel: "tel:0880600100", klant: "#/contact",
});
...
/* De routebeschrijving naar een vestiging als Google Maps-adres. Op een plek,
   omdat hij op twee plekken staat: onder het adres op de vestigingspagina en in
   de onderbalk die webbouw.cjs om diezelfde pagina heen zet. Twee keer dezelfde
   tekenreeks samenstellen is de manier waarop die twee ooit gaan verschillen. */
function routeUrl(l) {
  return `https://www.google.com/maps/dir/?api=1&destination=${
    encodeURIComponent(`${l.straat}, ${l.postcode} ${l.plaats}`)}`;
}

/* Het telefoonnummer van een vestiging als tel:-adres. Ook op een plek, om
   dezelfde reden als routeUrl: het staat op de kaart Contact en in de onderbalk.
   Alle tekens weg behalve cijfers en de plus is niet genoeg: Asten en Eindhoven
   staan in de database als "+31(0)493 670242" en "+31 (0) 40 262 02 22", en dan
   blijft de netnummer-nul achter +31 staan -- tel:+310493670242 gaat niet uit.
   Daarom de nul achter de landcode eraf; landelijk geschreven nummers (0493...)
   blijven zoals ze zijn. Zonder tel: geen lege knop maar het algemene nummer. */
function telHref(l) {
  const kaal = String(l.tel || "").replace(/[^0-9+]/g, "").replace(/^\+310/, "+31");
  return "tel:" + (kaal || "0880600100");
}

/* De vier knoppen van de onderbalk op een telefoon: Locatie, Route, Bel ons,
   Klant worden. Alleen de HTML binnen de <nav>; de nav zelf staat in
   template.html en in omhulsel() van webbouw.cjs, elk met eigen adressen.
   `naar` heeft de vier bestemmingen: {locatie, route, bel, klant}. De iconen
   staan hier een keer, zodat preview en site dezelfde balk tonen; ze tekenen
   met currentColor en krijgen hun geel uit de css. Een route naar buiten
   (Google Maps) opent in een nieuw tabblad, net als "Route plannen" op de
   pagina zelf. */
function onderbalk(naar) {
  const ico = {
    locatie: '<path d="M12 21.5s-7.2-6.4-7.2-11.4a7.2 7.2 0 0 1 14.4 0c0 5-7.2 11.4-7.2 11.4z"/><circle cx="12" cy="10" r="2.7"/>',
    route: '<circle cx="6" cy="18" r="2.6"/><circle cx="18" cy="6" r="2.6"/><path d="M8.6 18H14a3 3 0 0 0 0-6h-4a3 3 0 0 1 0-6h5.4"/>',
    bel: '<path d="M5.5 3.5h3.6l1.9 4.8-2.4 1.6a10.5 10.5 0 0 0 5.5 5.5l1.6-2.4 4.8 1.9v3.6a2 2 0 0 1-2.1 2A16.5 16.5 0 0 1 3.5 5.6a2 2 0 0 1 2-2.1z"/>',
    klant: '<circle cx="9.5" cy="8" r="3.6"/><path d="M2.8 20.5a6.7 6.7 0 0 1 13.4 0"/><path d="M19 8v6M16 11h6"/>',
  };
  const svg = d => `<svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${d}</svg>`;
  return [["locatie", "Locatie"], ["route", "Route"], ["bel", "Bel ons"], ["klant", "Klant worden"]]
    .map(([k, t]) => `<a href="${esc(naar[k])}"${/^https?:/.test(naar[k]) ? ' target="_blank" rel="noopener"' : ""}>${svg(ico[k])}<span>${t}</span></a>`)
    .join("");
}
...
// grove postcode -> coordinaat, genoeg om de dichtstbijzijnde vestiging te kiezen
...
function paginaHome() {
  ...
  const kies = n => d.find(x => x.slug.endsWith(n)) || d[0];
  return `
  <div class="hero">
    <canvas id="drie" aria-label="Driedimensionale weergave van een vrachtwagen in de wasstraat"></canvas>
    <div class="shell"><div class="hero-in">
      <span class="oogje"><span data-live="vestigingen">${DATA.locaties.length}</span> vestigingen in Nederland</span>
      <h1>Wassen zonder<br>afspraak. <em>Prijs<br>vooraf bekend.</em></h1>
      <p class="lead">Rij binnen bij de dichtstbijzijnde vestiging. Geen planning vooraf, geen offerte achteraf &mdash; je weet wat het kost voordat je de poort in draait.</p>
      <div class="zoeker">
        <label for="pc">Vind de dichtstbijzijnde vestiging</label>
        <div class="zoekrij">
          <input id="pc" type="text" inputmode="numeric" maxlength="7" autocomplete="postal-code" placeholder="Postcode, bijv. 5651" aria-describedby="uitkomst">
          <button class="knop knop-geel" id="zoekknop" type="button">Zoek</button>
        </div>
        <p class="uitkomst" id="uitkomst" role="status">Vul je postcode in om te zien waar je terechtkunt.</p>
      </div>
      <div class="hero-knoppen">
        <a class="knop knop-geel" href="#/prijzen">Bekijk alle prijzen</a>
        <a class="knop knop-rand" href="#/locaties">Alle vestigingen</a>
      </div>
      <div class="stats">
        <div class="stat"><b>18</b><span>vestigingen</span></div>
        <div class="stat"><b>4,7</b><span>gemiddeld op Google</span></div>
        <div class="stat"><b>7</b><span>dagen per week open*</span></div>
      </div>
    </div></div>
    <span class="drie-hint">Sleep om te draaien</span>
  </div>
  <div class="strook"></div>

  <section class="blok wit">
    <div class="shell">
      <div class="kopregel">
        <div>
          <p class="label">Prijsindicatie</p>
          <h2>Wat kost het?</h2>
          <p class="sub">Dezelfde prijslijst die aan de balie hangt. Kies je combinatie en je ziet meteen het bedrag.</p>
        </div>
        <a class="knop knop-navy" href="#/prijzen">Volledige prijslijst</a>
      </div>
      <div class="raster r2">
        <div class="kaart" style="padding:var(--s3)">
          <div class="veld">
            <label for="voertuig">Type voertuig</label>
            <select id="voertuig">${
              (DATA.prijstabellen[0] || []).slice(1).map(r =>
                `<option value="${esc(r[1])}">${esc(r[0])}</option>`).join("")}</select>
          </div>
          <div style="margin-top:var(--s3);padding-top:var(--s3);border-top:1px solid var(--lijn);display:flex;align-items:baseline;gap:12px;flex-wrap:wrap">
            <span id="bedrag" style="font-family:Archivo,sans-serif;font-weight:800;font-size:46px;line-height:1;color:var(--navy-700);font-variant-numeric:tabular-nums"></span>
            <span style="color:var(--inkt-3)">excl. 21% btw</span>
          </div>
          <p style="font-size:14.5px;color:var(--inkt-3);margin:14px 0 0">Toeslagen zoals Alcoa-behandeling of wax komen hier apart bij. Zie de volledige prijslijst.</p>
        </div>
        <div class="kaart" style="min-height:330px">
          <div class="beeld" style="aspect-ratio:auto;flex:1;min-height:0">${IMG("lanes", "Ingang van de wasstraat met basic lane en exclusive lane")}</div>
        </div>
      </div>
    </div>
  </section>

  <section class="wasrit" aria-labelledby="wasrit-kop">
    <div class="wasrit-spoor">
      <div class="wasrit-plakker">
        <canvas id="wascanvas" aria-hidden="true"></canvas>
        <div class="wasrit-hud">
          <p class="label">Zo gaat een wasbeurt</p>
          <h2 id="wasrit-kop">Van stoffig<br>naar spiegelend</h2>
          <ol class="wasrit-stappen">
            <li class="wasrit-stap aan"><b>Binnenrijden</b><span>Geen afspraak, geen wachtrij. Je rijdt de baan op en blijft zitten.</span></li>
            <li class="wasrit-stap"><b>Inschuimen</b><span>De hele wagen krijgt voorwas, zodat het vuil losweekt.</span></li>
            <li class="wasrit-stap"><b>Borstels</b><span>Zachte borstels nemen cabine, zijkanten en dak in één gang mee.</span></li>
            <li class="wasrit-stap"><b>Naspoelen</b><span>Spoelen, wax en drogen. Daarna rij je zo weer de weg op.</span></li>
          </ol>
          <div class="wasrit-balk" aria-hidden="true"><span></span></div>
        </div>
      </div>
    </div>
  </section>

  <section class="blok licht">
    <div class="shell">
      <p class="label">Waarom Truckwash 1</p>
      <h2>Meer dan schoon</h2>
      <p class="sub">Achttien vestigingen langs de belangrijkste corridors, met alles op één terrein zodat een chauffeur niet twee keer hoeft te stoppen.</p>
      <div class="raster r3">
        <a class="kaart" href="#/dienst/${esc(kies("alcoa-velgen-reinigen").slug)}">
          <div class="beeld">${IMG("alcoa", "Gepoetste Alcoa-velgen van een vrachtwagen")}</div>
          <div class="in"><h3>Alcoa-behandeling</h3><p>Velgen die weer glimmen in plaats van dof grijs. Per set geprijsd.</p><span class="meer">Bekijk &rarr;</span></div>
        </a>
        <a class="kaart" href="#/dienst/${esc(kies("interieur-reinigen").slug)}">
          <div class="beeld">${IMG("washal", "Oplegger in de washal")}</div>
          <div class="in"><h3>Inwendig reinigen</h3><p>Met of zonder HACCP-certificaat, afhankelijk van je lading.</p><span class="meer">Bekijk &rarr;</span></div>
        </a>
        <a class="kaart" href="#/dienst/${esc(kies("wasboxen").slug)}">
          <div class="beeld">${IMG("wasboxen", "Wasboxen om zelf te wassen")}</div>
          <div class="in"><h3>Zelf wassen</h3><p>Wasboxen per minuut, voor wie het liever zelf doet.</p><span class="meer">Bekijk &rarr;</span></div>
        </a>
        <a class="kaart" href="#/dienst/${esc(kies("truckparking").slug)}">
          <div class="beeld">${IMG("doetinchem", "Vestiging Doetinchem in de avond")}</div>
          <div class="in"><h3>Truckparking</h3><p>Overnachten op een bewaakt terrein, direct aan de snelweg.</p><span class="meer">Bekijk &rarr;</span></div>
        </a>
        <a class="kaart" href="#/dienst/${esc(kies("truck-shop").slug)}">
          <div class="beeld">${IMG("truckshop", "Truckshop met onderdelen en verzorgingsproducten")}</div>
          <div class="in"><h3>Truckshop</h3><p>Onderdelen, accessoires en verzorging, ook online.</p><span class="meer">Bekijk &rarr;</span></div>
        </a>
        <a class="kaart" href="#/dienst/${esc(kies("catering-op-locatie").slug)}">
          <div class="beeld">${IMG("restaurant", "Restaurant en vergaderruimte op locatie")}</div>
          <div class="in"><h3>Catering &amp; vergaderen</h3><p>Warme maaltijd voor de chauffeur, ruimte voor je team.</p><span class="meer">Bekijk &rarr;</span></div>
        </a>
      </div>
    </div>
  </section>

  <section class="blok donker" style="padding:0">
    <div class="band">
      <div class="foto">${IMG("wassen", "Medewerker wast de cabine van een vrachtwagen")}</div>
      <div class="tekst">
        <p class="label">Geen afspraak nodig</p>
        <h2>Binnenrijden<br>en klaar</h2>
        <p class="sub">Je hoeft niet te bellen, niet te plannen en niet te wachten op een offerte. Kom langs tijdens openingstijden en we helpen je.</p>
        <ul class="punten">
          <li>Betalen op rekening, pin, contant, Travis, CargoCard, DKV of Mastercard</li>
          <li>HACCP- en NAO-gecertificeerd inwendig reinigen</li>
          <li>Wachtruimte met koffie op elke vestiging</li>
        </ul>
        <a class="knop knop-geel" href="#/locaties">Vind een vestiging</a>
      </div>
    </div>
  </section>
  ...
  <section class="blok licht">
    <div class="shell">
      <div class="kopregel">
        <div><p class="label">Werken bij</p><h2>Gemotiveerde<br>aanpakkers gezocht</h2>
        <p class="sub">Geen diploma's of certificaten nodig &mdash; wel de wil om te leren. Bekijk de openstaande vacatures en het bruto uurloon.</p></div>
        <a class="knop knop-navy" href="#/werken-bij">Alle vacatures</a>
      </div>
      <div class="raster r4">
        ${["portret1","portret2","portret3","portret4"].map(p =>
          `<div class="kaart"><div class="beeld" style="aspect-ratio:1">${IMG(p, "Medewerker van Truckwash 1 Group")}</div></div>`).join("")}
      </div>
    </div>
  </section>`;
}

function paginaLocaties() {
  const L = DATA.locaties;
  return `
  <div class="detailkop">
    <div class="shell">
      <p class="label">Vestigingen</p>
      <h1 style="font-size:clamp(30px,4.4vw,50px);text-transform:uppercase;margin-bottom:var(--s2)"><span data-live="vestigingen">${DATA.locaties.length}</span> locaties door<br>heel Nederland</h1>
      <p class="sub" style="color:#b6c5e0;margin-bottom:var(--s4)">Sleep aan de kaart om hem te draaien, of klik een speld aan. Alle vestigingen zijn zonder afspraak bereikbaar.</p>
      <div class="kaartblok">
        <div class="kaarthouder">
          <canvas id="nlkaart" tabindex="0" role="img"
            aria-label="Driedimensionale kaart van Nederland met de achttien vestigingen. De volledige lijst met vestigingen staat hiernaast."></canvas>
          <span class="kaartlabel" aria-hidden="true"></span>
          <div class="kaartknoppen">
            <button type="button" data-kaart="in"  aria-label="Inzoomen">+</button>
            <button type="button" data-kaart="uit" aria-label="Uitzoomen">&#8722;</button>
            <button type="button" data-kaart="reset" aria-label="Kaart terugzetten">&#8635;</button>
          </div>
          <p class="kaarthint" aria-hidden="true">Sleep om te draaien &middot; scroll of knijp om te zoomen</p>
        </div>
        <div class="kaartnaast"><div class="loclijst">
          ${L.map((l, i) => `
            <a class="locrij" href="#/locatie/${esc(l.slug)}" data-i="${i}">
...
function paginaLocatie(s) {
  const l = DATA.locaties.find(x => x.slug === s);
  if (!l) return paginaOnbekend();
  /* Bij het statisch bouwen mag de open/dicht-status niet in de HTML gebakken
     worden: webbouw.cjs draait deze functie in een sandbox, dus new Date() is
     dan de bouwdag en die badge blijft daarna voor altijd staan. In de preview
     draait dezelfde functie in de browser en klopt hij wel. Op de gegenereerde
     pagina vult ijkOpeningstijden() hem bij het laden alsnog in. */
  const statisch = typeof STATISCH !== "undefined" && STATISCH;
  const st = openNu(l);
  const vandaag = statisch ? "" : DAGEN[new Date().getDay()];
  const badgeKlasse = statisch ? "" : (st.open ? "open" : "gesloten");
  const badgeTekst = statisch ? "Openingstijden hieronder" : st.tekst;
  const maps = routeUrl(l);
  const fotoRol = { eindhoven: "eindhoven", doetinchem: "doetinchem" }[l.slug] || "lanes";
  /* Foto's van de vestiging zelf, uit de database via omzet.cjs: [{url, alt}]
     met de omslagfoto voorop. Het veld kan ontbreken (oude vestigingen.json,
     site.json als terugval) of leeg zijn; dan staat er de stockfoto van
     vroeger en verder niets. De eerste foto neemt de plek van die stockfoto
     in, de rest wordt een rij vierkante tegels, hoogstens acht -- meer is een
     fotoalbum en geen vestigingspagina. Geen width/height: die zijn bij de
     bouw onbekend, de css houdt met aspect-ratio de plek vast. */
  const fotos = (Array.isArray(l.fotos) ? l.fotos : []).filter(f => f && f.url);
  const foto = f => `<img src="${esc(f.url)}" alt="${esc(f.alt || "Vestiging " + l.plaats)}" loading="lazy" decoding="async">`;
  const grootBeeld = fotos.length ? foto(fotos[0]) : IMG(fotoRol, "Vestiging " + l.plaats);
  const galerij = fotos.length > 1 ? `<div class="galerij">${fotos.slice(1, 9).map(f =>
    `<a href="${esc(f.url)}" target="_blank" rel="noopener">${foto(f)}</a>`).join("")}</div>` : "";
  return `
  <div class="detailkop">
    <div class="shell">
      <p class="kruimel"><a href="#/locaties">Vestigingen</a> &rsaquo; ${esc(l.plaats)}</p>
      <h1 style="font-size:clamp(32px,5vw,58px);margin-bottom:14px">Truckwash 1<br><em>${esc(l.plaats)}</em></h1>
      <p style="color:#c3d0e8;font-size:19px;margin:0 0 var(--s3)">${esc(l.straat)}, ${esc(l.postcode)} ${esc(l.plaats)}</p>
      <span class="nu ${badgeKlasse}"><i></i>${badgeTekst}</span>
    </div>
  </div>
  <section class="blok licht" style="padding-top:var(--s4)">
    <div class="shell">
      <div class="infoblok" style="margin-bottom:var(--s4)">
        <div class="infokaart" id="adres">
          <h4>Adres</h4>
          <p>${esc(l.straat)}</p><p>${esc(l.postcode)} ${esc(l.plaats)}</p>
          <p style="margin-top:14px"><a href="${esc(maps)}" target="_blank" rel="noopener">Route plannen &rarr;</a></p>
        </div>
        <div class="infokaart">
          <h4>Contact</h4>
          <p><a href="${esc(telHref(l))}">${esc(l.tel)}</a></p>
          <p><a href="mailto:${esc(l.email)}">${esc(l.email)}</a></p>
          <p style="margin-top:14px;color:var(--inkt-3);font-size:15px">Geen afspraak nodig</p>
        </div>
        <div class="infokaart">
          <h4>Openingstijden</h4>
          <table class="uren"><tbody>${
            l.uren.map(u => `<tr class="${u.dag === vandaag ? "vandaag" : ""}"><td>${esc(u.dag)}</td>
              <td class="${!u.tijd || /gesloten/i.test(u.tijd) ? "dicht" : ""}">${esc(u.tijd || "Gesloten")}</td></tr>`).join("")}
          </tbody></table>
        </div>
      </div>
      ${l.urenVol && l.urenVol.length ? `<div class="kaart" style="padding:var(--s3);margin-bottom:var(--s4)">
        <h4 style="font-size:13px;letter-spacing:.12em;text-transform:uppercase;color:var(--inkt-3);margin:0 0 14px;font-weight:600">Openingstijden per onderdeel</h4>
        <div class="tblwrap"><table>
          <thead><tr><th>Dag</th>${l.urenKoppen.map(k => `<th>${esc(k)}</th>`).join("")}</tr></thead>
          <tbody>${l.urenVol.map(r => `<tr><td>${esc(r.dag)}</td>${
            l.urenKoppen.map((_, i) => `<td>${esc(r.tijden[i] || "—")}</td>`).join("")}</tr>`).join("")}</tbody>
        </table></div>
      </div>` : ""}
      ${l.intro ? `<div class="band" style="background:var(--wit);border:1px solid var(--lijn);border-radius:4px;overflow:hidden">
        <div class="foto">${grootBeeld}</div>
        <div class="tekst">
          <h2 style="font-size:28px;margin-bottom:var(--s2)">Over deze vestiging</h2>
          <p style="color:var(--inkt-2);margin-bottom:var(--s2)">${esc(l.intro)}</p>
          ${l.extra ? `<p style="color:var(--inkt-2)">${esc(l.extra)}</p>` : ""}
          ${l.diensten && l.diensten.length ? `<ul class="punten" style="margin-top:var(--s2)">${
            l.diensten.map(x => `<li>${esc(x)}</li>`).join("")}</ul>` : ""}
          <a class="knop knop-geel knop-klein" href="#/prijzen">Prijzen bekijken</a>
        </div>
      </div>` : (fotos.length ? `<div class="band" style="background:var(--wit);border:1px solid var(--lijn);border-radius:4px;overflow:hidden">
        <div class="foto">${grootBeeld}</div>
        <div class="tekst">
          <h2 style="font-size:28px;margin-bottom:var(--s2)">Truckwash 1 ${esc(l.plaats)}</h2>
          <a class="knop knop-geel knop-klein" href="#/prijzen">Prijzen bekijken</a>
        </div>
      </div>` : "")}
      ${galerij}
...
function paginaPrijzen() {
  const T = DATA.prijstabellen;
  const titels = T.map(t => (t[0] && t[0][0]) || "Tarieven");
  return `
  <div class="detailkop">
    <div class="shell">
      <p class="label">Tarieven 2026</p>
      <h1 style="font-size:clamp(32px,5vw,56px);margin-bottom:14px">Prijzen, <em>gewoon<br>zichtbaar</em></h1>
      <p style="color:#c3d0e8;font-size:19px;max-width:56ch;margin:0">Alle tarieven zijn exclusief 21% btw. Zoek op voertuigtype of blader per categorie.</p>
    </div>
  </div>
  <section class="blok licht" style="padding-top:var(--s4)">
    <div class="shell">
      <div class="prijsbalk">
        <input id="prijszoek" type="search" placeholder="Zoek op voertuig, bijv. trekker of oplegger" aria-label="Zoek in de prijslijst">
        ${titels.map((t, i) => `<button class="chip" data-tab="${i}" aria-pressed="${i === 0}">${esc(t)}</button>`).join("")}
      </div>
      <p class="telling" id="telling"></p>
      ${T.map((t, i) => `
        <div class="tabelblok" data-tab="${i}" style="${i ? "display:none" : ""}">
          <div class="tblwrap"><table>
            <thead><tr><th>${esc(t[0][0] || "Omschrijving")}</th><th class="bedrag">${esc(t[0][1] || "Tarief")}</th></tr></thead>
            <tbody>${t.slice(1).map(r =>
              `<tr><td>${esc(r[0])}</td><td class="bedrag">${esc(r[1])}</td></tr>`).join("")}</tbody>
          </table></div>
        </div>`).join("")}
      <p style="color:var(--inkt-3);font-size:15px;margin-top:var(--s3)">
        Prijzen overgenomen van truckwash1group.nl op 31 augustus 2026. Aan deze weergave kunnen geen rechten worden ontleend.</p>
    </div>
  </section>`;
}

function paginaDiensten() {
  const beelden = ["washal","wasboxen","doetinchem","truckshop","restaurant","hogedruk",
                   "verzorging","camper","vakwerk","eten","alcoa","lanes","wassen","eindhoven"];
  return `
  <div class="detailkop">
    <div class="shell">
      <p class="label">Diensten en behandelingen</p>
      <h1 style="font-size:clamp(32px,5vw,56px);margin-bottom:14px">Alles op<br><em>één terrein</em></h1>
      <p style="color:#c3d0e8;font-size:19px;max-width:56ch;margin:0">Van een gewone wasbeurt tot HACCP-gecertificeerd inwendig reinigen, parkeren en een warme maaltijd.</p>
    </div>
  </div>
  <section class="blok licht" style="padding-top:var(--s4)">
    <div class="shell">
      <div class="raster r3">
        ${DATA.diensten.map((s, i) => `
          <a class="kaart" href="#/dienst/${esc(s.slug)}">
            <div class="beeld">${IMG(beelden[i % beelden.length], s.titel)}</div>
            <div class="in">
              <h3>${esc(s.titel)}</h3>
              <p>${esc((s.tekst[0] || "").slice(0, 130))}${(s.tekst[0] || "").length > 130 ? "&hellip;" : ""}</p>
              <span class="meer">${s.groep === "behandeling" ? "Behandeling" : "Dienst"} &rarr;</span>
            </div>
          </a>`).join("")}
      </div>
    </div>
...
function paginaDienst(s) {
  const d = DATA.diensten.find(x => x.slug === s);
  if (!d) return paginaOnbekend();
  const i = DATA.diensten.indexOf(d);
  const beelden = ["washal","wasboxen","doetinchem","truckshop","restaurant","hogedruk",
                   "verzorging","camper","vakwerk","eten","alcoa","lanes","wassen","eindhoven"];
  return `
  <div class="detailkop">
    <div class="shell">
      <p class="kruimel"><a href="#/diensten">Diensten</a> &rsaquo; ${esc(d.titel)}</p>
      <h1 style="font-size:clamp(30px,4.6vw,52px)">${esc(d.titel)}</h1>
    </div>
  </div>
  <section class="blok licht" style="padding-top:var(--s4)">
    <div class="shell">
      <div class="band" style="background:var(--wit);border:1px solid var(--lijn);border-radius:4px;overflow:hidden">
        <div class="foto">${IMG(beelden[i % beelden.length], d.titel)}</div>
        <div class="tekst">
          ${d.tekst.map(p => `<p style="color:var(--inkt-2);margin-bottom:var(--s2)">${esc(p)}</p>`).join("")}
          ${d.punten.length ? `<ul class="punten">${d.punten.map(p => `<li>${esc(p)}</li>`).join("")}</ul>` : ""}
          <div class="rij" style="margin-top:var(--s2)">
            <a class="knop knop-geel knop-klein" href="#/locaties">Vind een vestiging</a>
            <a class="knop knop-navy knop-klein" href="#/prijzen">Prijzen</a>
          </div>
        </div>
      </div>
    </div>
...
function paginaWerkenBij() {
  /* Hoeveel mensen er bij Truckwash 1 Group werken. Komt uit
     website_aantal_medewerkers(), via vestigingen.json en webbouw.cjs.

     Bij de bouw vastleggen en niet in de browser ophalen: die databasefunctie
     is alleen voor service_role, dus elke paginaweergave zou langs een open
     edge function moeten voor een getal dat een paar keer per jaar verandert --
     met een kop die na het laden verspringt, en helemaal niets voor een
     bezoeker zonder JavaScript. De val die de open/dicht-badge brak geldt hier
     niet: dit getal doet geen enkele datumberekening, het veroudert tussen twee
     bouwen en niet binnen een bouw. Wat wel overslaat van die badge is het
     taalgebruik. Geen "op dit moment", "inmiddels" of "vandaag": die woorden
     claimen actualiteit terwijl de tekst op de bouwdag in de HTML gaat.

     Is het getal er niet, dan komt de hele alinea er niet -- geen streepje,
     geen lege regel. De kop is dan byte voor byte de kop van vandaag, en dat is
     meteen de toestand waarvan bewezen is dat hij klopt. Onder de twee valt hij
     ook weg: 0 is geen bedrijf maar een kapotte query, en bij 1 zou er "werken
     er 1 mensen" staan.

     De typeof-guard is niet optioneel. In de preview (template.html) bestaat
     deze variabele niet, en daar hoort de pagina eruit te zien zoals de
     terugval. */
  const N = typeof MEDEWERKERS !== "undefined" &&
    Number.isInteger(MEDEWERKERS) && MEDEWERKERS >= 2 ? MEDEWERKERS : 0;
  return `
  <div class="detailkop">
    <div class="shell">
      <p class="label">Werken bij Truckwash 1 Group</p>
      <h1 style="font-size:clamp(32px,5vw,56px);margin-bottom:14px">Een leuke en<br><em>actieve job</em></h1>
      <p style="color:#c3d0e8;font-size:19px;max-width:56ch;margin:0">Geen diploma's of certificaten nodig. Wel de wil om te leren en te groeien.</p>${N ? `
      <p style="color:#c3d0e8;font-size:19px;max-width:56ch;margin:var(--s2) 0 0">Er werken <span data-live="medewerkers">${N}</span> mensen bij Truckwash 1 Group. Word jij de volgende?</p>` : ""}
    </div>
  </div>
  <section class="blok licht" style="padding-top:var(--s4)">
    <div class="shell">
      <p class="label">Openstaande vacatures</p>
      <h2 style="margin-bottom:var(--s4)">Vier functies</h2>
      <div class="raster" style="gap:12px">
        ${DATA.vacatures.map(v => `
          <div class="vacrij">
            <div class="vi">
              <h3>${esc(v.titel)}</h3>
              <p>${esc((v.intro || "").slice(0, 150))}${(v.intro || "").length > 150 ? "&hellip;" : ""}</p>
            </div>
            <a class="knop knop-navy knop-klein" href="#/vacature/${esc(v.slug)}">Bekijk vacature</a>
          </div>`).join("")}
      </div>
    </div>
  </section>
  <section class="blok wit">
    <div class="shell">
      <div class="band" style="gap:var(--s4)">
        <div class="foto" style="border-radius:4px;overflow:hidden">${IMG("hero", "Team van Truckwash 1 Group in de wasstraat")}</div>
        <div class="tekst" style="padding:0">
          <p class="label">Wat je krijgt</p>
          <h2 style="font-size:32px;margin-bottom:var(--s2)">Doorgroeien kan hier echt</h2>
          <p style="color:var(--inkt-2);margin-bottom:var(--s2)">Meerdere van onze vestigingsleiders begonnen ooit met een zaterdagbaan in de wasstraat. Wij kijken naar inzet, niet naar papieren.</p>
          <ul class="punten">
            <li>Bruto uurloon volgens een vast leeftijdsoverzicht</li>
            <li>Werken vanuit één van de achttien vestigingen</li>
            <li>Een informele werksfeer in een groeiend bedrijf</li>
          </ul>
          <a class="knop knop-geel" href="#/vacature/open-sollicitatie">Direct solliciteren</a>
        </div>
      </div>
    </div>
  </section>`;
...
/* ------------------------------------------------------------------
   ...
   Stond hiervoor bij truckwash.trainstation.nl. Wat daar binnenkwam kwam
   nergens in ons eigen systeem terecht: geen melding, geen taak, geen
   dossier. En bij de pendelchauffeur wees de knop naar vacaturenummer 1 --
   hetzelfde nummer als de washeld -- zodat een sollicitatie op de verkeerde
   vacature belandde.

   De vragen komen uit het gespreksformulier van Casper. Niet alle vragen
   daaruit: dat is een template voor het GESPREK, en de helft hoort daar ook
   te blijven. "Algemene indruk", de proefdag en de doorgroeimogelijkheden
   vult de gespreksvoerder in; wat een sollicitant zelf kan beantwoorden staat
   hier.

   Eén vraag is bewust anders gesteld. Het formulier vraagt "wat is je
   leeftijd", en dat is een antwoord dat na een verjaardag niet meer klopt --
   terwijl het loon uit de salaristabel eraan hangt. Dus: geboortedatum.

   Waarom de vestiging verplicht is: een sollicitatie zonder vestiging komt
   bij niemand op het bord. De leiding van Venlo ziet het werk van Venlo, en
   zonder plaats is er geen leiding om het aan te geven.
   ------------------------------------------------------------------ */

function sollicitatieFormulier(v) {
  /* De vacature zegt op welke vestigingen hij openstaat. Staat daar niets --
     omdat de vacature overal geldt, of omdat de lijst nog uit site.json komt
     en niet uit de database -- dan mag je uit alle achttien kiezen. Een lege
     keuzelijst zou betekenen dat niemand kan solliciteren, en dat is het
     ergste dat dit formulier kan doen. */
  const plaatsen = (Array.isArray(v.locaties) && v.locaties.length)
    ? v.locaties
    : DATA.locaties.map(l => ({ slug: l.slug, plaats: l.plaats }));
  const dagen = ["Maandag", "Dinsdag", "Woensdag", "Donderdag", "Vrijdag", "Zaterdag"];

  return `
  <section class="blok wit" id="solliciteren">
    <div class="shell" style="max-width:760px">
      <p class="label">Solliciteren</p>
      <h2 style="font-size:clamp(26px,3.6vw,40px);margin-bottom:10px">Laat weten wie je bent</h2>
      <p style="color:var(--inkt-2);margin-bottom:var(--s3)">
        Je hebt geen motivatiebrief nodig. Vul in wat je kwijt wilt &mdash; alleen de
        velden met een sterretje zijn verplicht. We nemen binnen een paar
        werkdagen contact op.
      </p>

      <form class="sol-form" id="solform" data-vacature="${esc(v.slug)}" novalidate>
        <div class="sol-rij">
          <label>Je naam *<input name="naam" required autocomplete="name" maxlength="120"></label>
          <label>E-mailadres *<input name="email" type="email" required autocomplete="email" maxlength="160"></label>
        </div>
        <div class="sol-rij">
          <label>Telefoonnummer<input name="telefoon" type="tel" autocomplete="tel" maxlength="40"></label>
          <label>Geboortedatum<input name="geboortedatum" type="date"></label>
        </div>
        <div class="sol-rij">
          <label>Woonplaats<input name="woonplaats" autocomplete="address-level2" maxlength="80"></label>
          <label>Op welke vestiging? *
            <select name="locatie" required>
              <option value="">Kies een vestiging</option>
              ${plaatsen.map(l => `<option value="${esc(l.slug)}">${esc(l.plaats)}</option>`).join("")}
            </select>
          </label>
        </div>

        <h3 class="sol-kop">School en werk</h3>
        <div class="sol-rij">
          <label>Zit je op school of studeer je?
            <select name="school">
              <option value="">Zeg het maar</option>
              <option value="ja">Ja</option>
              <option value="nee">Nee</option>
            </select>
          </label>
          <label>Zo ja: welke opleiding?<input name="opleiding" maxlength="120"></label>
        </div>
        <div class="sol-rij">
          <label>Welk niveau?<input name="niveau" maxlength="60" placeholder="bijv. mbo 3"></label>
          <label>Welk leerjaar?<input name="leerjaar" maxlength="40"></label>
        </div>
        <label>Heb je al eerder in deze branche gewerkt?
          <textarea name="ervaring" rows="2" maxlength="800"
                    placeholder="Wasstraat, transport, horeca, klantenservice&hellip; en waar."></textarea>
        </label>
        <div class="sol-rij">
          <label>Hoe ben je bij ons terechtgekomen?
            <select name="hoeGevonden">
              <option value="">Zeg het maar</option>
              <option>Vacaturesite</option>
              <option>Via een kennis of medewerker</option>
              <option>Social media</option>
              <option>Langs de vestiging</option>
              <option>Anders</option>
            </select>
          </label>
          <label>Hoe lang denk je dit te willen doen?
            <select name="hoeLang">
              <option value="">Zeg het maar</option>
              <option>Naast school, voorlopig</option>
              <option>Tijdelijk, een paar maanden</option>
              <option>Ik zoek vast werk</option>
              <option>Naast ander werk</option>
            </select>
          </label>
        </div>
        <label>Is er iets dat je inzet op termijn kan beperken?
          <textarea name="beperkingen" rows="2" maxlength="500"
                    placeholder="Afstuderen, stage, verhuizing, ander werk&hellip;"></textarea>
        </label>

        <h3 class="sol-kop">Vervoer</h3>
        <div class="sol-rij">
          <label>Hoe kom je naar het werk?
            <select name="vervoer">
              <option value="">Zeg het maar</option>
              <option>Eigen auto</option>
              <option>Scooter of brommer</option>
              <option>Fiets</option>
              <option>Openbaar vervoer</option>
              <option>Word gebracht</option>
            </select>
          </label>
          <label>Heb je een rijbewijs?
        ...
        <div class="sol-rij">
          <label>Hoe lang ben je onderweg?<input name="reistijd" maxlength="40" placeholder="bijv. 20 minuten"></label>
          <label>Zou je ook op een andere vestiging kunnen bijspringen?
            <select name="andereVestiging">
              <option value="">Zeg het maar</option>
              <option value="ja">Ja, dat kan</option>
              <option value="nee">Liever niet</option>
            </select>
          </label>
        </div>

        <h3 class="sol-kop">Wanneer kun je?</h3>
        <p style="color:var(--inkt-3);font-size:14px;margin:-6px 0 10px">
          Laat leeg wat niet van toepassing is. Al onze vestigingen zijn op zondag gesloten.
        </p>
        <div class="sol-dagen">
          ${dagen.map(d => `
            <div class="sol-dag" data-dag="${d.toLowerCase()}">
              <span>${d}</span>
              <input type="time" name="van" aria-label="${d} vanaf">
              <input type="time" name="tot" aria-label="${d} tot">
              <input name="opmerking" maxlength="120" placeholder="bijv. school tot 15:00"
                     aria-label="${d} opmerking">
            </div>`).join("")}
        </div>

        <label class="sol-motivatie">Wil je verder nog iets kwijt?
          <textarea name="motivatie" rows="4" maxlength="2000"
                    placeholder="Vertel kort wie je bent en waarom je hier wilt werken."></textarea>
        </label>

        <p class="sol-klein">
          We gebruiken wat je invult alleen om je sollicitatie te behandelen.
          <a href="/privacy/">Zo gaan we met je gegevens om</a>.
        </p>

        <div class="sol-knoppen">
          <button class="knop knop-geel" type="submit">Sollicitatie versturen</button>
          <p class="sol-melding" id="solmelding" role="status"></p>
        </div>
      </form>
    </div>
  </section>`;
...
function paginaVacature(s) {
  const v = DATA.vacatures.find(x => x.slug === s);
  if (!v) return paginaOnbekend();
  const stuk = st => st.type === "kop"
    ? `<h3 style="margin:var(--s4) 0 var(--s2);font-size:23px">${esc(st.tekst)}</h3>`
    : st.type === "lijst"
      ? `<ul class="punten">${st.punten.map(p => `<li>${esc(p)}</li>`).join("")}</ul>`
      : `<p style="color:var(--inkt-2);margin-bottom:var(--s2)">${esc(st.tekst)}</p>`;
  return `
  <div class="detailkop">
    <div class="shell">
      <p class="kruimel"><a href="#/werken-bij">Werken bij</a> &rsaquo; ${esc(v.titel)}</p>
      <h1 style="font-size:clamp(30px,4.6vw,52px);margin-bottom:14px">${esc(v.titel)}</h1>
      <a class="knop knop-geel" href="#solliciteren">Solliciteer direct</a>
    </div>
  </div>
  <section class="blok licht" style="padding-top:var(--s4)">
    <div class="shell" style="max-width:860px">
      ${v.inhoud.map(stuk).join("")}
    </div>
  </section>
  ${sollicitatieFormulier(v)}`;
}

function paginaOverOns() {
  const o = DATA["over-ons"] || { tekst: [] };
  const hoe = DATA["hoe-het-begon"] || { tekst: [] };
  const vis = DATA["toekomst-en-visie"] || { tekst: [] };
  const duu = DATA["over-ons_duurzaamheid"] || { tekst: [] };
  return `
  <div class="detailkop">
    <div class="shell">
      <p class="label">Over ons</p>
      <h1 style="font-size:clamp(32px,5vw,56px);margin-bottom:14px">De standaard in<br><em>vrachtwagens wassen</em></h1>
    </div>
  </div>
  <section class="blok licht" style="padding-top:var(--s4)">
    <div class="shell">
      <div class="band" style="background:var(--wit);border:1px solid var(--lijn);border-radius:4px;overflow:hidden;margin-bottom:var(--s4)">
        <div class="foto">${IMG("team", "De leiding van Truckwash 1 Group")}</div>
        <div class="tekst">
          <h2 style="font-size:30px;margin-bottom:var(--s2)">Wie we zijn</h2>
          ${(o.tekst.length ? o.tekst : hoe.tekst).slice(0, 3).map(p =>
            `<p style="color:var(--inkt-2);margin-bottom:var(--s2)">${esc(p)}</p>`).join("")}
        </div>
      </div>
      <div class="raster r2">
        <div class="kaart"><div class="in">
          <p class="label">Hoe het begon</p>
          ${hoe.tekst.slice(0, 2).map(p => `<p>${esc(p)}</p>`).join("")}
        </div></div>
        <div class="kaart"><div class="in">
          <p class="label">Toekomst en visie</p>
          ${vis.tekst.slice(0, 2).map(p => `<p>${esc(p)}</p>`).join("")}
        </div></div>
      </div>
      ${duu.tekst.length ? `<div class="kaart" style="margin-top:var(--s2)"><div class="in">
        <p class="label">Duurzaamheid</p>
        ${duu.tekst.slice(0, 2).map(p => `<p>${esc(p)}</p>`).join("")}
      </div></div>` : ""}
    </div>
  </section>
  <section class="blok wit">
    <div class="shell">
      ...
      <h2 style="margin-bottom:var(--s4)">Kort antwoord op<br>de meeste vragen</h2>
      <div class="faq">${DATA.faq.slice(0, 8).map(f => `
        <details><summary>${esc(f.v)}</summary><div class="antwoord">${esc(f.a)}</div></details>`).join("")}
      </div>
      <p style="margin-top:var(--s3)"><a href="#/faq">Alle vragen bekijken &rarr;</a></p>
    </div>
  </section>`;
}
...
function paginaFaq() {
  ...
  <div class="detailkop">
    <div class="shell">
      <p class="label">Veelgestelde vragen</p>
      <h1 style="font-size:clamp(32px,5vw,56px)">Vragen en<br><em>antwoorden</em></h1>
    </div>
  </div>
  <section class="blok licht" style="padding-top:var(--s4)">
    <div class="shell" style="max-width:860px">
      <div class="faq">${DATA.faq.map(f => `
        <details><summary>${esc(f.v)}</summary><div class="antwoord">${esc(f.a)}</div></details>`).join("")}
      </div>
      <p style="margin-top:var(--s4);color:var(--inkt-2)">Staat je vraag er niet bij?
        <a href="#/contact">Neem contact op</a> of bel <a href="tel:0880600100">088 &ndash; 0600 100</a>.</p>
    </div>
  </section>`;
}
...
function paginaContact() {
  ...
  <div class="detailkop">
    <div class="shell">
      ...
      <h1 style="font-size:clamp(32px,5vw,56px);margin-bottom:14px">Neem gerust<br><em>contact op</em></h1>
      <p style="color:#c3d0e8;font-size:19px;max-width:52ch;margin:0">Vragen over je wagenpark, een vaste afspraak of gewoon de openingstijden? We helpen je graag.</p>
    </div>
  </div>
  <section class="blok wit" id="klant-worden" style="padding-top:var(--s4);padding-bottom:var(--s4)">
    <div class="shell">
      <div class="klantblok">
        <div>
          <p class="label">Klant worden</p>
          <h2 style="margin-bottom:var(--s2)">Zakelijk wassen,<br>op rekening</h2>
          <p style="color:var(--inkt-2);margin:0 0 var(--s2)">Voor transportbedrijven met een wagen of een hele vloot. Je meldt je bedrijf hier aan, wij bellen je terug om de afspraken vast te leggen, en daarna rijdt elke chauffeur gewoon binnen.</p>
          <ul class="punten">
            <li>Vast tarief per wasbeurt, vooraf bekend</li>
            <li>Betalen op factuur achteraf is mogelijk</li>
            <li>Ook met CargoCard of DKV</li>
            <li>Zonder afspraak bij alle ${DATA.locaties.length} vestigingen</li>
          </ul>
          <p style="color:var(--inkt-3);font-size:15px;margin:0">Liever bellen? <a href="tel:0880600100">088 &ndash; 0600 100</a>, op werkdagen.</p>
        </div>
        <div class="kaart formkaart">
          <form class="form" id="klantform">
            <div class="veld"><label for="kbedrijf">Bedrijf</label><input id="kbedrijf" autocomplete="organization" maxlength="160" required></div>
            <div class="veld"><label for="knaam">Contactpersoon</label><input id="knaam" autocomplete="name" maxlength="120" required></div>
            <div class="veld"><label for="kmail">E-mail</label><input id="kmail" type="email" autocomplete="email" required></div>
            <div class="veld"><label for="ktel">Telefoon</label><input id="ktel" type="tel" autocomplete="tel" maxlength="40" required></div>
            <div class="veld"><label for="kwagens">Aantal wagens</label><input id="kwagens" type="number" inputmode="numeric" min="1" max="9999" step="1" required></div>
            <div class="veld"><label for="kvest">Vestiging van voorkeur</label>
              <select id="kvest"><option value="">Geen voorkeur</option>
                ${DATA.locaties.map(l => `<option>${esc(l.plaats)}</option>`).join("")}</select></div>
            <div class="veld vol"><label for="kopm">Opmerking (mag leeg)</label><textarea id="kopm" maxlength="1500" style="min-height:90px" placeholder="Bijvoorbeeld: welke wagens, hoe vaak, of wanneer we het best kunnen bellen."></textarea></div>
            <div class="vol"><button class="knop knop-geel" type="submit">Aanmelding versturen</button></div>
          </form>
          <p class="klantmelding" id="klantmelding" role="status"></p>
        </div>
      </div>
    </div>
  </section>
  <section class="blok licht" style="padding-top:var(--s4)">
    <div class="shell">
      <div class="infoblok" style="margin-bottom:var(--s4)">
        <div class="infokaart"><h4>Telefoon</h4>
          <p><a href="tel:0880600100" style="font-size:21px;font-weight:700">088 &ndash; 0600 100</a></p>
          <p style="color:var(--inkt-3);font-size:15px">Voor alle vestigingen</p></div>
        <div class="infokaart"><h4>E-mail</h4>
          <p><a href="mailto:info@truckwash1group.nl">info@truckwash1group.nl</a></p>
          <p><a href="mailto:administratie@truckwash1group.nl">administratie@truckwash1group.nl</a></p></div>
        <div class="infokaart"><h4>Correspondentieadres</h4>
          <p>Industriestraat 28</p><p>4715 RL Rucphen</p></div>
      </div>

      <div class="kaart formkaart">
        <h2 style="font-size:28px;margin-bottom:var(--s2)">Stuur een bericht</h2>
        <p style="color:var(--inkt-2);margin-bottom:var(--s3)">Vul in wat je wil weten. Je eigen mailprogramma opent met het bericht klaar om te versturen.</p>
        <form class="form" id="contactform">
          <div class="veld"><label for="cnaam">Naam</label><input id="cnaam" required></div>
          <div class="veld"><label for="cbedrijf">Bedrijf</label><input id="cbedrijf"></div>
          <div class="veld"><label for="cmail">E-mail</label><input id="cmail" type="email" required></div>
          <div class="veld"><label for="ctel">Telefoon</label><input id="ctel" type="tel"></div>
          <div class="veld vol"><label for="cvest">Vestiging</label>
            <select id="cvest"><option value="">Geen voorkeur</option>
              ${DATA.locaties.map(l => `<option>${esc(l.plaats)}</option>`).join("")}</select></div>
          <div class="veld vol"><label for="cbericht">Bericht</label><textarea id="cbericht" required></textarea></div>
          <div class="vol"><button class="knop knop-geel" type="submit">Bericht opstellen</button></div>
        </form>
      </div>
    </div>
  </section>`;
...
const REL_APP = "https://truckwash-workspace.com/app/";
...
function paginaMedewerkers() {
  /* RELEASES komt uit bouw/releases.json en wordt door webbouw.cjs in de
     sandbox gezet. In de preview bestaat hij niet; dan is de lijst leeg en
     wijst alles naar de releasepagina op GitHub. Precies dezelfde toestand als
     wanneer releases.json ontbreekt, dus er is maar een codepad. */
  const REL = typeof RELEASES !== "undefined" && RELEASES ? RELEASES : [];
  const nieuwste = REL[0] || null;
  const lijstUrl = REL_REPO + "/releases";

  /* De id's zijn het haakje waar vulReleases() in de browser aan trekt: die zet
     er het adres en de naam van de dan nieuwste versie in. Staat er geen
     gebakken bestand, dan wijst de knop naar de releasepagina -- die werkt
     altijd, ook zonder JavaScript. */
  const knop = (asset, id, klasse, label) => asset
    ? `<a class="knop ${klasse}" id="${id}" href="${esc(asset.url)}">${label}</a>
          <span class="dlgrootte" id="${id}-info">${esc(asset.naam)} &middot; ${esc(relGrootte(asset.bytes))}</span>`
    : `<a class="knop ${klasse}" id="${id}" href="${esc(lijstUrl)}">Naar de downloads</a>
          <span class="dlgrootte" id="${id}-info">Kies daar het nieuwste bestand.</span>`;

  const rij = r => {
    const dl = [
      r.win ? `<a href="${esc(r.win.url)}">Windows</a>` : "",
      r.apk ? `<a href="${esc(r.apk.url)}">Android</a>` : "",
    ].filter(Boolean).join("");
    return `<div class="relrij">
          <div class="vi">
            <h3>${esc(r.versie)}<span class="reldatum">${esc(relDatumNl(r.datum))}</span>${
              r === nieuwste ? `<span class="relnu">nieuwste</span>` : ""}</h3>
            ${r.kop ? `<p>${esc(r.kop)}</p>` : ""}
          </div>
          <div class="reldl">${dl}</div>
        </div>`;
  };
  ...
  <div class="detailkop">
    <div class="shell">
      <p class="label">Voor medewerkers</p>
      <h1 style="font-size:clamp(32px,5vw,56px);margin-bottom:14px">De app waar<br><em>het werk in staat</em></h1>
      <p style="color:#c3d0e8;font-size:19px;max-width:54ch;margin:0 0 var(--s3)">Roosters, uren, wasbeurten, storingen en overleg. Op de computer van de vestiging, op de tablet in de wasstraat, of gewoon in de browser.</p>
      <p style="display:flex;flex-wrap:wrap;gap:12px;margin:0 0 var(--s2)">
        <a class="knop knop-geel" href="${esc(REL_APP)}">Inloggen op de werkplek</a>
      </p>
      <p style="color:#9fb0d0;font-size:15px;margin:0">Inloggen kan altijd in de browser. Hieronder staat de app voor Windows en Android.</p>
    </div>
  </div>

  <section class="blok licht" style="padding-top:var(--s4)" id="downloaden">
    <div class="shell">
      <p class="label">Downloaden</p>
      <h2 style="margin-bottom:var(--s2)">Op welk apparaat<br>wil je hem?</h2>
      <p class="sub">De app is van onszelf en staat in geen enkele appwinkel. Je haalt hem hier op.</p>
      <p class="relmelding" id="relmelding" hidden></p>
      <p class="relnoot" id="relstand">${nieuwste
        ? `Versie ${esc(nieuwste.versie)}, uitgebracht op ${esc(relDatumNl(nieuwste.datum))}. Dit is de stand van de laatste sitebouw; je browser kijkt na of er inmiddels een nieuwere is.`
        : `De versies worden bij GitHub opgehaald.`}</p>
      <div class="infoblok">
        <div class="infokaart"><h4>Windows</h4>
          <p>Voor de computer op de vestiging en op kantoor.</p>
          <p>${knop(nieuwste && nieuwste.win, "dl-win", "knop-geel", "Windows-installer")}</p>
          <p style="color:var(--inkt-2);font-size:15.5px">Dubbelklikken en de installatie volgen. Daarna hoef je niets meer te doen: de app kijkt zelf of er een nieuwe versie is en installeert die bij het afsluiten.</p></div>
        <div class="infokaart"><h4>Android</h4>
          <p>Voor de telefoon en de tablet in de wasstraat.</p>
          <p>${knop(nieuwste && nieuwste.apk, "dl-apk", "knop-navy", "Android (APK)")}</p>
          <p style="color:var(--inkt-2);font-size:15.5px">Zet het bestand op je toestel en tik erop. Je toestel vraagt eenmalig of het mag installeren uit een onbekende bron. Dat komt doordat de app van ons komt en niet uit de Play Store.</p>
          <p style="color:var(--inkt-3);font-size:15px">Een nieuwe versie haal je hier zelf op; de Android-app werkt zichzelf nog niet bij.</p></div>
        <div class="infokaart"><h4>In de browser</h4>
          <p>Werkt op elk apparaat, zonder installeren.</p>
          <p><a class="knop knop-navy" href="${esc(REL_APP)}">Open de app</a></p>
          <p style="color:var(--inkt-2);font-size:15.5px">Handig als je even achter een andere computer zit. Herladen geeft de nieuwste versie. Op een iPhone of iPad is dit voorlopig de manier: daar is een account bij Apple voor nodig en dat is er nog niet.</p></div>
      </div>
    </div>
  </section>
  ...
  <section class="blok wit">
    <div class="shell">
      <p class="label">Wat je ermee doet</p>
      <h2 style="margin-bottom:var(--s2)">Alles van een dag<br>op een plek</h2>
      <p class="sub">Je logt in en kiest je rol. Daarna zie je een raster tegels met wat er te doen is, met op elke tegel een cijfer dat leeft. Je ziet alleen wat bij jou hoort: jouw vestiging en jouw rol.</p>
      <div class="raster r3">
        <div class="kaart"><div class="in"><h3>Werknemer</h3>
          <p>De wasopdrachten van vandaag en de wachtrij, wagens oppakken en gereed melden. In- en uitklokken, met je week- en maandtotaal. Materiaal afboeken. Je rooster, je dossier en je loonstroken. En een bon indienen die daarna beoordeeld wordt.</p></div></div>
        <div class="kaart"><div class="in"><h3>Leidinggevende</h3>
          <p>Wie er vandaag staat en wie is ingeklokt. Het rooster maken en publiceren. Smartroster stelt een hele week voor op basis van contracturen, de tijden die iemand gewoonlijk werkt en de drukte van die dag, met per regel de reden erbij.</p></div></div>
        <div class="kaart"><div class="in"><h3>Technische dienst</h3>
          <p>Storingen, werkbonnen, installaties en onderhoud. Elk apparaat heeft een QR-label met een sleutel die er ook leesbaar onder staat, want in een natte machinekamer is intypen soms sneller dan scannen. Een werkbon afronden sluit de storing en schuift het onderhoudsschema door.</p></div></div>
        <div class="kaart"><div class="in"><h3>Administratie</h3>
          <p>Alles wat op een beslissing wacht op een scherm: kostenposten, urenwijzigingen, dossierwijzigingen en aanmeldingen.</p></div></div>
        <div class="kaart"><div class="in"><h3>Management</h3>
          <p>Omzet, kosten en volume over 7, 30 of 90 dagen, met de vorige periode ernaast. Kosten valideren of afkeuren met reden. Personeel, planning, voorraad, techniek, en per persoon instellen wat hij mag en over welke vestigingen hij gaat.</p></div></div>
      </div>

      <p class="label" style="margin-top:var(--s4)">Overal in de app</p>
      <div class="infoblok">
        <div class="infokaart"><h4>Zoeken met Ctrl+K</h4>
          <p>Door wasbeurten, klanten, medewerkers, voorraad en cursussen. Je ziet alleen waar je bij mag. Met natte handschoenen aan klik je op de microfoon en zeg je wat je zoekt.</p></div>
        <div class="infokaart"><h4>Ook zonder bereik</h4>
          <p>Alles wat je ziet komt van het apparaat zelf. Wat je in de machinekamer invult staat er meteen en gaat vanzelf weg zodra je weer buiten staat.</p></div>
        <div class="infokaart"><h4>Overleg</h4>
          <p>Kanalen per onderwerp, een kanaal per vestiging, en rechtstreekse gesprekken. Je krijgt alleen bericht als iemand je noemt of je rechtstreeks aanschrijft, niet bij elke regel.</p></div>
        <div class="infokaart"><h4>Storing melden</h4>
          <p>De knop staat in de balk van elk scherm, want wie het defect ziet is meestal de wasser en niet de monteur. Vier niveaus, en een schakelaar voor: de installatie ligt stil.</p></div>
        <div class="infokaart"><h4>Opleiding</h4>
          <p>Veiligheid, chemie, machines, kwaliteit en klantcontact. Met toets, slaagnorm en certificaten die verlopen.</p></div>
        <div class="infokaart"><h4>Iets kapot in de app</h4>
          <p>Naast Storing staat Devmelding. Wat je invult gaat samen met je apparaat en je versie naar de ontwikkelaar. Dat spoor zie je voor het versturen.</p></div>
      </div>
      <p style="color:var(--inkt-3);font-size:15px;margin-top:var(--s3)">De eerste keer dat je inlogt rijdt er een vrachtwagen door de wasstraat voordat je binnen bent. Dat duurt een paar tellen en is rechtsboven over te slaan.</p>
    </div>
  </section>

  <section class="blok licht" id="versies">
    <div class="shell">
      <p class="label">Versies</p>
      <h2 style="margin-bottom:var(--s2)">Wat er per versie<br>is veranderd</h2>
      <p class="sub">Een regel per versie, die zegt wat er is aangepakt. Wil je een oudere versie terughalen, dan staan de bestanden er meteen naast.</p>
      <div class="rellijst" id="rellijst" data-nieuwste="${esc(nieuwste ? nieuwste.versie : "")}">
        ${REL.length ? REL.map(rij).join("\n        ")
          : `<p class="relnoot" id="relleeg">De lijst wordt opgehaald. Lukt dat niet, dan staan alle versies op GitHub.</p>`}
      </div>
      <p style="margin-top:var(--s3)"><a href="${esc(lijstUrl)}" target="_blank" rel="noopener">Alle versies en bestanden op GitHub &#8599;</a></p>
    </div>
  </section>

  <section class="blok">
    <div class="shell">
      <p class="label">Inloggen</p>
      <h2 style="margin-bottom:var(--s2)">Je logt in met<br>je werkmail</h2>
      <div class="infoblok">
        <div class="infokaart"><h4>Je hebt al een account</h4>
          <p>Vul je e-mailadres en wachtwoord in. Onderaan het inlogscherm staat welke versie je draait, handig als je iets moet doorgeven.</p>
          <p><a class="knop knop-navy knop-klein" href="${esc(REL_APP)}">Naar het inlogscherm</a></p></div>
        <div class="infokaart"><h4>Je hebt nog geen account</h4>
          <p>Onder het inlogscherm staat Aanmelden. Je vult in wie je bent, op welke vestiging je werkt en met welk e-mailadres. Daarna beoordeelt het management je aanmelding en krijg je bericht zodra je erin kunt. Tot dat moment kom je nergens binnen. Dat is geen storing.</p></div>
        <div class="infokaart"><h4>Er klopt iets niet</h4>
          <p>Wachtwoord kwijt, verkeerde vestiging, of een scherm dat je zou moeten zien maar niet ziet? Vraag het je leidinggevende of het management: zij zetten je rollen, je rechten en je vestiging goed.</p></div>
      </div>
      <p style="color:var(--inkt-3);font-size:15px;margin-top:var(--s3)">Geen bereik op de vestiging? Je kunt offline inloggen met een account dat eerder op dat apparaat is gebruikt.</p>
    </div>
  </section>`;
}
...
function paginaOnbekend() {
  return `<section class="blok licht"><div class="shell" style="text-align:center;padding:var(--s6) 0">
    <h1 style="font-size:64px;color:var(--navy-700)">404</h1>
    <p class="sub" style="margin:0 auto var(--s3)">Deze pagina bestaat niet.</p>
    <a class="knop knop-geel" href="#/">Terug naar de homepage</a>
  </div></section>`;
}
...
const ROUTES = [
  [/^#?\/?$/,                 paginaHome,      "#/"],
  [/^#\/locaties$/,           paginaLocaties,  "#/locaties"],
  [/^#\/locatie\/(.+)$/,      paginaLocatie,   "#/locaties"],
  [/^#\/prijzen$/,            paginaPrijzen,   "#/prijzen"],
  [/^#\/diensten$/,           paginaDiensten,  "#/diensten"],
  [/^#\/dienst\/(.+)$/,       paginaDienst,    "#/diensten"],
  [/^#\/werken-bij$/,         paginaWerkenBij, "#/werken-bij"],
  [/^#\/vacature\/(.+)$/,     paginaVacature,  "#/werken-bij"],
  [/^#\/over-ons$/,           paginaOverOns,   "#/over-ons"],
  [/^#\/faq$/,                paginaFaq,       "#/over-ons"],
  [/^#\/contact$/,            paginaContact,   "#/contact"],
  [/^#\/medewerkers$/,        paginaMedewerkers, "#/medewerkers"],
];

function teken() {
  const h = location.hash || "#/";
  let html = paginaOnbekend(), actief = null;
  for (const [re, fn, menu] of ROUTES) {
    const m = h.match(re);
    if (m) { html = fn(m[1] ? decodeURIComponent(m[1]) : undefined); actief = menu; break; }
  }
  const app = document.getElementById("app");
  app.innerHTML = html;
  app.style.animation = "none"; void app.offsetWidth; app.style.animation = "";
  document.querySelectorAll("#menu a").forEach(a =>
    a.setAttribute("aria-current", a.getAttribute("href") === actief ? "page" : "false"));
...
function naTekenen(h) {
  // hero 3D
  const cv = document.getElementById("drie");
  if (cv) requestAnimationFrame(() => bouwHero3D(cv));
  const wc = document.getElementById("wascanvas");
  if (wc) requestAnimationFrame(() => bouwWas3D(wc));
  onthulBijScroll();
  telOp();
  ...
  if (pc) {
    ...
    const zoek = () => {
      ...
      uit.innerHTML = `Dichtstbijzijnd: <a href="#/locatie/${esc(best.slug)}">${esc(best.plaats)}</a>
        &mdash; ${esc(best.straat)} &middot; <span class="km">ca. ${Math.round(d)} km</span><br>
  ...
  if (kv) {
    const rijen = [...document.querySelectorAll(".locrij")];
    requestAnimationFrame(() => bouwKaart3D(kv, DATA.locaties, i => {
      location.hash = "#/locatie/" + DATA.locaties[i].slug;
    }));
    rijen.forEach(r => {
  ...
  if (zoekveld) {
    ...
    chips.forEach(c => c.addEventListener("click", () => {
      tab = +c.dataset.tab; zoekveld.value = ""; ververs();
    }));
  ...
  if (cf) cf.addEventListener("submit", e => {
    ...
    const body = [
      `Naam: ${w("cnaam")}`, `Bedrijf: ${w("cbedrijf")}`, `E-mail: ${w("cmail")}`,
      `Telefoon: ${w("ctel")}`, `Vestiging: ${w("cvest") || "geen voorkeur"}`,
      "", w("cbericht"),
    ].join("\n");
    location.href = "mailto:info@truckwash1group.nl?subject=" +
      encodeURIComponent("Bericht via de website - " + w("cnaam")) +
      "&body=" + encodeURIComponent(body);
  });

  // klant worden op /contact/ -> de trucky-functie, net als het chatformulier
  klantWordenFormulier();

  // versienummers en downloadknoppen op /medewerkers/ bijwerken
  vulReleases();
}
